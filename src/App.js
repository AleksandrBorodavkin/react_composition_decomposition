...
function App() {
    ...
    const newsItems = [
        {
            ...
            title: 'Поздравления с Новым годом!'
        },
        {
            icon: 'https://via.placeholder.com/15x15/',
            title: 'Где отпраздновать 2021'
        },
        {
            icon: 'https://via.placeholder.com/15x15/',
            title: 'Что подарить семье?'
        },
        {
            icon: 'https://via.placeholder.com/15x15/',
    ...
    const advertising = {
        icon: 'https://via.placeholder.com/70x70/',
        ...
        <div className="App">
            <header className='header-navigation'>
                <div className='nav-left'>
                    <Nav className="" >
                        {newsListCategory.map(item => <NavItem><NavLink
                            disabled={item.active ? '' : 'disabled'}
                            href="#">{item.title}</NavLink></NavItem>)}
            ...
            <main>
                <Nav className='nav-searchCategory'>
                    {nav.map(item => <NavItem><NavLink
                        disabled={item.active ? '' : 'disabled'}
                        href="#">{item.title}</NavLink></NavItem>)}
                </Nav>
                <SearchForm/>
                <div className='advertising-section'>
                    <img src='https://via.placeholder.com/1000x70/'/>
                </div>
                <WidgetList>
                    ...
                    <Widget title='Посещаемое'
                            className='widget widget-visited'>
                        {favorite.map(item =>
                            <div>
                    ...
                    <Widget title='Карта Краснодара'
                            className='widget widget-map'>
                        <a href='#'>Расписания</a>
                    </Widget>
                    <Widget title='Телепрограмма'
                            className='widget widget-tv-program'>
                        {tvProgram.map(item =>
                            <div>
                                <span>{item.time}</span>
                                <span>{item.name}</span>
